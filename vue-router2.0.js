 ...
 (function (global, factory) {
  ...
  var Link = {
    ...
    render: function render (h) {
      ...
      var router = this.$router // 是 VueRouter 实例
      ...
      var handler = function (e) {
        // router-link 点击处理事件，做了啥
        if (guardEvent(e)) {
          if (this$1.replace) {
            router.replace(normalizedTo)
          } else {
            router.push(normalizedTo) // 把当前 router-link 的路由信息推入历史栈
          }
        }
      }
  ...
  History.prototype.confirmTransition = function confirmTransition (route, cb) {  
    ...
    this.pending = route
    var iterator =  (hook /** 当前元素 */, next) => {
      if ( this.pending !== route) return 
      hook(route/** from */, current/** to */, to => { /** next() */
        if (to === false) {
    ...
    function runQueue (queue, fn /** 单个元素执行 */, cb /** 全部执行完再执行 */) {
      ...
      function step (index) {
        if (index >= queue.length) {
          ...
          if (queue[index]) {
            fn/** iterator */(queue[index], function () {
              step(index + 1)
            })
          } else {
            step(index + 1)
  ...
  function flatMapComponents (
    ...
    return flatten(
      matched.map(m => Object.keys(m.components)
        .map(key => fn(
          m.components[key],
          m.instances[key],
          m, key
